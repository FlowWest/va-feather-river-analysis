...
```{r}
# count fish observation per microhabitat plot
...
mini_snorkel_model_ready |> 
  ...
  tally() |> 
  mutate(percent = (n/1235)*100) |> 
  ggplot(aes(x = cover_percent, y = n)) +
  geom_col() +
  facet_wrap(~cover)   
```
...
This trend also applies if use number of fish observations (present/absent) rather than absolute number of fish observed.
