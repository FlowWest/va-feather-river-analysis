...
#hurdle_model <- hurdle(count ~ ., data = test) # , zero.dist = "binomial", dist = "negbin"

# zero inflated models are capable of dealing with excess zero counts
hurdle_model <- zeroinfl(count ~ ., data = test) # , zero.dist = "binomial", dist = "negbin"

summary(hurdle_model)

best_model <- stepAIC(hurdle_model, trace = FALSE)

summary(best_model)

pred_binary <- predict(best_model, type = "response", newdata = test)
pred_count <- predict(best_model, type = "count", newdata = test)
# Combine the predicted values
pred_total <- pred_binary * pred_count

all <- cbind(pred_total, test)

# explore: https://stackoverflow.com/questions/74440752/is-there-a-way-to-plot-hurdle-model-results-pscl-package-or-a-way-to-plot-the-co
ggplot(all[all$count > 0, ], aes(x = count, y = round(pred_total, 0))) +
  geom_point() +
  stat_smooth(method = "glm", se = TRUE)

ggplot(all |> select(percent_undercut_bank, count, pred_total), aes(x = count, y = pred_total)) + 
  geom_point()
