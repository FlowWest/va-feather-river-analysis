...
This visual represents the number of redds over time at each location. It helps provide context on which sites generally have redds and if they have redds consistently over time. The Mini Snorkel data was collected from `r min(mini_snorkel_raw$date)` to `r max(mini_snorkel_raw$date)` so it does not align with the redd data. 
...
known to be important for salmon rearing habitat. We are also including the number or redds nearby, when applicable. 
...
AUC of `r auc(roc_glm)` means The habitat variables explain some presence–absence structure, but there is still a lot of overlap/noise in the data.
