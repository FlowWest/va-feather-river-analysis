...
This plot justifies the use of 50-meter redd distance
```{r}
...
This mixed-effects logistic model estimates the probability of fish presence as a function of local habitat features and spawning context, while accounting for consistent differences among sites and channel types through random effects. 

We first fit a mixed-effects logistic regression that included a random effect for channel type (high-flow versus low-flow channels). We then extended this structure by adding a nested random effect for transect sites within each channel type. The nested model showed slightly improved performance based on AUC, indicating better overall discrimination between presence and absence. However, the confusion matrices for the two models were identical, suggesting that while the nested random effects improved probabilistic ranking of sites, they did not change binary classification outcomes at the chosen threshold.
...
After accounting for local habitat characteristics, fish occurrence remained strongly structured by site and channel context. The mixed-effects logistic models revealed substantial spatial variation in baseline occurrence probabilities, with low-flow channels (LFC) consistently supporting higher occupancy and high-flow channels (HFC) supporting lower occupancy than expected based on measured habitat alone. Random-intercept estimates for channel units nested within locations indicated that some sites exhibited persistently elevated probabilities of fish presence, whereas others were characterized by consistently reduced occurrence. For example, hatchery ditch (LFC) and junkyard riffle (HFC) showed markedly positive random effects—corresponding to approximately two- to ten-fold higher odds of presence relative to the global mean—while eye side channel (LFC) and weir riffle (LFC) exhibited substantially lower occurrence (approximately 60–70% lower odds). These patterns suggest the influence of additional reach-scale or contextual factors not captured by the measured microhabitat variables, such as longitudinal connectivity, geomorphic setting, or broader hydraulic and thermal conditions.
