---
title: "Feather River - Logistic Regression Analysis"
output:
  flexdashboard::flex_dashboard:
    ...
    navbar:
      - { title: "Cover Analysis Dashboard", href: "mini-snorkel-cover-analysis-dashboard.Rmd", align: left }
runtime: shiny
---
...
channel_colors <- c(HFC = "#1f78b4", LFC = "#33a02c")
...
build_formula <- function(species_choice, random_effects = NULL) {
  extra  <- if (species_choice == "steelhead trout" &&
                "surface_turbidity" %in% names(mini_fish_raw))
...
<style>
/* Remove fixed heights imposed by flexdashboard on chart containers and tab panes */
.chart-wrapper,
.chart-stage,
.chart-shim,
.tab-content,
...
hr()
p(em("Data: Feather River Mini Snorkel Survey (EDI, 2001–2002)"))
p(em("Model: Mixed-effects logistic regression (glmmTMB)"))
hr()
p(strong("Note:"), "Switching species refits all models — allow ~15 seconds.")
```
...
```{r overview_obj}
renderUI({
  sp <- if (input$species == "chinook salmon") "Chinook Salmon" else "Steelhead Trout"

  tagList(
    fluidRow(
      column(7,
        h4(paste("Objective —", sp)),
        p("Develop a model that reflects the significance of cover, substrate, depth, and
           velocity on", tolower(sp), "presence and absence in the Feather River using
           Mini Snorkel Survey data (EDI, 2001–2002)."),
        hr(),
        h4("Modeling approach"),
        tags$ol(
          tags$li(strong("Initial: Hurdle model"), "— evaluated to separate zero/non-zero
                   counts. Count component performed poorly due to extreme zero-inflation
                   and high variability. Analysis refocused on presence/absence."),
          tags$li(strong("Refined: Logistic regression"), "— binary response (fish present/absent).
                   Cover and substrate variables binarized at a",
                  strong(paste0(percent_threshold, "% threshold")), "."),
          tags$li(strong("Mixed-effects structure"), "— glmmTMB with random intercepts for
                   transect location and month to capture spatial heterogeneity and seasonal dynamics.")
        ),
        hr(),
        h5("Best model result"),
        if (input$species == "chinook salmon") {
          tagList(
            p("Random effects for", strong("transect location"), "and",
              strong("month (all survey months, March–August)"), "— AUC ≈ 0.91.")
          )
        } else {
          tagList(
            p("Random effects for", strong("transect location"), "and",
              strong("month (March–May, active rearing period)"), "— AUC ≈ 0.91.")
          )
        }
      ),
      column(5,
        ...
        renderUI({
          ...
          tags$table(
            ...
            tags$tbody(
              ...
              tags$tr(tags$th("P/A threshold"),      tags$td(paste0(percent_threshold, "%")))
            )
          )
        }),
        br(),
        ...
        renderUI({
          ...
          tags$ol(
            ...
            tags$li("RE: location + month",
                    if (sp2 == "chinook salmon") strong(" ← best") else NULL),
            if (sp2 == "steelhead trout")
              tags$li("RE: location + month (March–May)", strong(" ← best"))
            else NULL
          )
        })
      )
...
renderPlot({
  d        <- species_data()
  sp_label <- if (input$species == "chinook salmon") "Chinook Salmon" else "Steelhead Trout"

  p1 <- d$raw |>
    filter(count > 0) |>
    ggplot(aes(count, fill = channel_location)) +
    geom_histogram(binwidth = 50, color = "white") +
    scale_fill_manual(values = custom_colors, name = "Channel") +
    facet_grid(~ channel_location) +
    labs(title = paste("Count distribution —", sp_label, "(non-zero observations)"),
...
renderUI({
  sp    <- if (input$species == "chinook salmon") "Chinook Salmon" else "Steelhead Trout"
  extra <- if (input$species == "steelhead trout")
    tags$li(strong("Surface turbidity"), " — categorical") else NULL

  tagList(
    h4(paste("Predictor variables —", sp)),
    p("Cover and substrate variables are measured as percentages and converted to binary
       ...
       Overhanging vegetation categories (½ m and >½ m) were combined into a single variable."),
    hr(),
    fluidRow(
      column(4,
        h5("Continuous"),
      ...
      column(4,
        h5(paste0("Binary (≥", percent_threshold, "%)")),
        tags$ul(
      ...
      column(4,
        ...
        h5("Response"),
        tags$ul(
          tags$li(strong("Fish presence"), " — 1 if count > 0, else 0")
        )
      )
    )
  )
...
renderPlot(height = 750, {
  ...
  p2 <- d$raw |>
    ...
    ggplot(aes(x = as.factor(month), y = pct_present, fill = channel_location)) +
    geom_col(position = "dodge") +
    scale_fill_manual(values = channel_colors, name = "Channel") +
    labs(title = "% transects with fish present by month and channel",
         x = "Month", y = "% observations with fish") +
    theme_minimal(base_size = 13)
  ...
  p3 <- d$raw |>
    mutate(overhanging_veg = percent_cover_half_meter_overhead +
                             percent_cover_more_than_half_meter_overhead) |>
    select(channel_location, fish_presence, all_of(names(cover_vars_raw)), overhanging_veg) |>
    rename(!!!setNames(names(cover_vars_raw), unname(cover_vars_raw)), `Overhanging veg` = overhanging_veg) |>
    pivot_longer(c(all_of(unname(cover_vars_raw)), "Overhanging veg"),
                 names_to = "feature", values_to = "pct") |>
    ggplot(aes(x = pct, fill = fish_presence)) +
    geom_histogram(binwidth = 5, color = "white", position = "stack") +
    geom_vline(xintercept = percent_threshold, linetype = 2, color = "red", linewidth = 0.6) +
    scale_fill_manual(
      values = c("0" = "gray70", "1" = "#D55E00"),
      labels = c("0" = "Absent", "1" = "Present"),
      name   = paste(sp_label, "presence")
    ) +
    facet_wrap(~ feature, ncol = 4, scales = "free_y") +
    labs(
      title   = paste("Raw % cover distributions —", sp_label),
      caption = paste0("Red dashed line = ", percent_threshold, "% binarization threshold"),
      x = "Raw percent cover (%)", y = "Count"
    ) +
...
renderUI({
  ...
  wellPanel(
    selectInput("selected_site", "Select a site:", choices = sites, selected = sites[1]),
    p(em("Toggle between sites to see species presence patterns and cover composition."))
  )
})
```

```{r site_explorer_plots}
renderPlot({ 
  req(input$selected_site)
  d        <- species_data()
  sp_label <- if (input$species == "chinook salmon") "Chinook Salmon" else "Steelhead Trout"
  fill_col <- if (input$species == "chinook salmon") "#0072B2" else "#009E73"
  ...
  p1 <- ggplot(site_monthly, aes(x = month, y = pct_present)) +
    ...
    geom_text(aes(label = paste0("n=", n_obs)), vjust = -0.4, size = 3.2, color = "gray40") +
    scale_y_continuous(limits = c(0, max(site_monthly$pct_present, 5) * 1.2)) +
    labs(
      title = paste(sp_label, "presence rate by month —", tools::toTitleCase(input$selected_site)),
      x = "Month", y = "% observations with fish"
    ) +
    theme_minimal(base_size = 13)
  ...
  cover_df <- d$raw |>
    ...
    mutate(overhanging_veg = percent_cover_half_meter_overhead +
                             percent_cover_more_than_half_meter_overhead) |>
    select(all_of(names(cover_raw)), overhanging_veg) |>
    rename(!!!setNames(names(cover_raw), unname(cover_raw)), `Overhanging veg` = overhanging_veg) |>
  ...
  p2 <- ggplot(cover_df, aes(x = mean_pct, y = reorder(feature, mean_pct))) +
    ...
    geom_vline(xintercept = percent_threshold, linetype = 2, color = "red", linewidth = 0.7) +
    labs(
      title   = paste("Mean % cover —", tools::toTitleCase(input$selected_site)),
      x       = "Mean % cover",
      y       = NULL,
      caption = paste0("Red line = ", percent_threshold, "% binarization threshold")
    ) +
    theme_minimal(base_size = 13)
...
<!-- ═══════════════════════════════════════════════════════════════════════════ -->
<!-- TAB 3 · MODEL BUILDING & PERFORMANCE                                        -->
<!-- ═══════════════════════════════════════════════════════════════════════════ -->
...
renderDT({
  ...
  datatable(
    ...
    formatStyle("AUC",
      background = styleInterval(c(0.7, 0.8, 0.9),
                                 c("white", "#fff3cd", "#d4edda", "#cce5ff"))
    ) |>
    formatStyle("Sensitivity",
      background = styleInterval(c(0.1, 0.2, 0.3),
                                 c("white", "#fff3cd", "#d4edda", "#cce5ff"))
    )
})
```
...
renderPlot({
  ...
  roc_to_df <- function(m) {
    r <- m$roc
    data.frame(
      FPR   = 1 - r$specificities,
      TPR   = r$sensitivities,
      model = paste0(m$label, " (AUC = ", round(auc(r), 3), ")")
    )
  }
  ...
  ggplot(df, aes(x = FPR, y = TPR, color = model)) +
    ...
    labs(
      title = paste("ROC curves —", sp_label),
...
renderPlot({
  ...
  term_labels <- c(
    ...
    depth             = "Depth",
    velocity          = "Velocity",
    redd_total        = "Total redds nearby",
    redd_presence     = "Redd present",
  ...
  ggplot(effects_df, aes(x = estimate, y = reorder(term_label, estimate),
    ...
    scale_color_manual(
      values = c(`FALSE` = "gray60", `TRUE` = "#D55E00"),
    ...
    labs(
      title   = paste("Predictor effect sizes —", sp_label),
...
renderPlot({
  ...
  ggplot(month_df, aes(x = odds_ratio, y = unit)) +
    ...
    scale_x_log10() +
    labs(
      title   = paste("Month random effects —", sp_label),
      caption = "OR > 1: higher presence probability relative to global mean",
      x       = "Random-effect odds ratio (log scale)",
      y       = NULL
    ) +
    theme_minimal(base_size = 13)
})
...
renderPlot({
  ...
  ggplot(re_loc, aes(x = odds_ratio, y = unit, color = channel_location)) +
    ...
    scale_x_log10() +
    labs(
      title   = paste("Site random effects —", sp_label),
...
renderPlot({
  d        <- species_data()
  sp_label <- if (input$species == "chinook salmon") "Chinook Salmon" else "Steelhead Trout"
  fill_col <- if (input$species == "chinook salmon") "darkblue" else "darkgreen"
...
renderUI({
  ...
  cover_finding <- if (input$species == "chinook salmon") {
    tagList(
    ...
    tagList(
      p(strong("Overhanging vegetation"), ",", strong("aquatic vegetation"), ", and",
        strong("small woody debris"), "were all positively associated with juvenile
         steelhead presence — consistent with use of structurally complex microhabitats
         for predation risk reduction."),
      p(strong("Velocity"), "was negative.
         Multiple cover types contribute, rather than a single dominant predictor,
         suggesting steelhead use a broader range of cover features than Chinook.")
    )
  }

  tagList(
    h4(paste("Cover is important —", sp)),
    p(em("Finding: Cover increases the probability of juvenile fish presence, but its
           effect is modulated by spatial and temporal context.")),
    hr(),
    cover_finding,
    hr(),
    h5("Literature context"),
    p("There is a paucity of habitat suitability literature where cover is treated
       consistently. Where it does exist, cover has a variable influence on rearing
       juvenile salmonids — positive associations are documented, but effect sizes
       vary with channel type, fish size, and season."),
    tags$ul(
      tags$li(strong("FERC 2004:"), "Positive association between overhanging vegetation
               and juvenile salmonid presence using the same Feather River Mini Snorkel data.
               No random effects; no predictive validation."),
      tags$li(strong("Beakes et al. 2012:"), "Cover included as a composite suitability
               index component; AUC = 0.65, Kappa = 0.20 — lower than this study's AUC ≈ 0.91."),
      tags$li(strong("This study:"), "Mixed-effects logistic regression with full predictive
               evaluation confirms that cover — especially overhanging vegetation — is a
               robust and significant predictor of juvenile presence in the Feather River.")
    )
  )
})
```

### Study Design & Skewness
...
renderUI({
  ...
  tagList(
    h4("Study design and data skewness"),
    fluidRow(
      column(6,
        h5("Zero-inflation and class imbalance"),
        p(paste0("Fish presence records comprise only ", prevalence, "% of all observations (",
                 format(n_present, big.mark = ","), " of ", format(n_obs, big.mark = ","), " total). ",
        ...
        tags$ul(
          ...
          tags$li("Sensitivity (correctly detecting true presences) is low at the default
                   0.5 classification threshold."),
          tags$li("A hurdle model was evaluated for count data but performed poorly — count
                   variability was too high and zero-inflation too severe for reliable inference.
                   Analysis was refocused on binary presence/absence.")
        )
      ),
      column(6,
        h5("Temporal mismatch"),
        p("Mini Snorkel surveys were conducted in 2001–2002. Redd data span 2014–2023 (Chinook)
           and ongoing (Steelhead). The redd-fish spatial correlation is interpreted as a",
          em("spatial pattern only"), "— not a contemporaneous relationship.
           Redd density reflects long-run spawning use of a reach, not a snapshot
           coincident with the fish surveys."),
        hr(),
        h5("Binary cover variables"),
        p("Cover and substrate variables measured as percentages were binarized at a",
          strong(paste0(percent_threshold, "% threshold")), "for modeling. This simplification
           trades some information for interpretability and stability. Alternative thresholds
           would change classification of individual observations but key predictors remain
           consistent across threshold values tested."),
        hr(),
        h5("Survey design"),
        p("Fixed transect snorkel surveys across March–August, conducted by CDFW.
           Not all sites were surveyed in all months — random effects for month help account
           for this unbalanced sampling structure.")
      )
    )
  )
})
...
renderUI({
  ...
  tagList(
    ...
    p(em("Finding: Juvenile fish presence is spatially associated with redd locations,
      ...
      column(6,
        ...
        tags$ul(
          ...
          tags$li(strong("Binary redd presence (redd_presence):"),
                  "positive but not always significant — redd",
                  em("intensity"), "(count) is more informative than simple
                   presence/absence of spawning.")
        ),
        br(),
        h5("Mechanisms"),
        tags$ol(
          tags$li(strong("Parent-offspring proximity:"), "Juveniles emerge and rear near
                   where adults spawned. Reaches with higher spawning intensity produce more
                   juveniles locally."),
          tags$li(strong("Shared habitat quality:"), "Reaches that attract spawning adults
                   (clean gravel, appropriate velocity and depth) may also provide good rearing
                   habitat — the correlation partly reflects shared suitability rather than
                   a strict parent-offspring link.")
        )
      ),
      column(6,
        h5("When the coupling breaks down"),
        p("The redd-juvenile coupling is not universal. Site random effects show high variability
           in baseline presence probabilities", em("independent"), "of redd counts.
           Some sites have high spawning activity but low juvenile rearing use (or vice versa)."),
        p("Factors that may mediate or decouple the relationship:"),
        tags$ul(
          ...
          tags$li("Competition and density dependence — high spawning-area density
                   may not translate to proportionally higher juvenile counts if
                   competition for cover or food limits capacity."),
          tags$li("River size and project footprint — larger reaches have greater
                   spatial heterogeneity in redd-to-rearing habitat proximity.")
        )
      )
    )
  )
...
renderUI({
  ...
  tagList(
    h4(paste("Connection with hypotheses —", sp)),
    p("The analysis was structured around three hypotheses. Below is an assessment of
       the evidence for each based on model results."),
    hr(),

    div(class = "panel panel-info",
      div(class = "panel-heading",
          h5(style = "margin: 0", "H1 — Cover in the literature")),
      div(class = "panel-body",
        p(em("There is a paucity of habitat suitability literature and where it does exist,
              cover is treated differently and has a variable influence on rearing juvenile salmonids.")),
        p(strong("Assessment: Supported.")),
        tags$ul(
          tags$li("Few studies report full predictive evaluations (AUC, confusion matrix,
                   Kappa) alongside traditional association metrics."),
          tags$li("Where cover is included, its operationalization varies: percentage,
                   binary, categorical, or composite index — making cross-study comparisons difficult."),
          tags$li("Effect sizes and significance of cover variables vary with channel type,
                   fish size class, season, and whether hydraulics are co-modeled.")
        )
      )
    ),
    br(),
    ...
    div(class = "panel panel-success",
      div(class = "panel-heading",
          h5(style = "margin: 0", "H2 — Cover increases presence on the Feather River")),
      div(class = "panel-body",
        p(em("For the Feather River dataset, cover increases the presence of juvenile salmon.")),
        p(strong("Assessment: Supported.")),
        tags$ul(
          tags$li("Overhanging vegetation is the strongest and most consistent cover predictor
                   across both species, model formulations, and channel types."),
          tags$li("For steelhead, small woody debris and aquatic vegetation also contribute positively."),
          tags$li("Cover effects are robust to inclusion of hydraulic variables (depth, velocity)
                   and random effects for site and month.")
        )
      )
    ),
    br(),
    ...
    div(class = "panel panel-warning",
      div(class = "panel-heading",
          h5(style = "margin: 0", "H3 — Multiple factors mediate the impact of cover")),
      div(class = "panel-body",
        p(em("Juvenile salmon presence is driven by multiple factors that vary in time and space
              (related to where adults spawn), and these factors mediate the impact of cover.")),
        p(em("Sub-hypotheses: (a) Cover suitability may change through time and be mediated by
              depth/fish growth; (b) River size and project footprint influence patterns.")),
        p(strong("Assessment: Partially supported.")),
        tags$ul(
          tags$li("Strong month and site random effects confirm that temporal and spatial context
                   substantially modulate fish distribution — cover alone does not explain all variance."),
          tags$li("Redd density is a significant predictor, linking spawning adult distribution
                   to juvenile rearing location (spatial mediation)."),
          tags$li("Depth is a significant fixed-effect predictor, consistent with the idea
                   that habitat suitability shifts with fish size through the growing season."),
          tags$li("The interaction between cover and these contextual factors is not explicitly
                   modeled — it is implicitly captured in the random-effects structure, which absorbs
                   reach-scale variation not explained by fixed effects.")
        )
      )
    )
  )
