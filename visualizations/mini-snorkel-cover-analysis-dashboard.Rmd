...
feather_cover <- mini_fish_raw |> 
  ...
  pivot_longer(cols = c(small_woody_debris, boulder, cobble, undercut_bank, aquatic_veg, overhanging_veg, depth, velocity), names_to = "cover_type", 
...
summary_cover <- feather_cover |> 
  ...
  summarize(mean_percent_cover = mean(percent_cover, na.rm = T),
            median_percent_cover = median(percent_cover, na.rm = T),
            min_percent_cover = min(percent_cover, na.rm = T),
            max_percent_cover = max(percent_cover, na.rm = T)) |> 
...
```{r}
# TODO: need to add redds 
imp_steelhead_pred <- c('redds_nearby', 'overhanging_veg', 'aquatic_veg', 'small_woody_debris', 'total_redds', 'depth', 'velocity')

imp_chinook_pred <- c("overhanging_veg", "depth", "total_redds", "velocity")
```
