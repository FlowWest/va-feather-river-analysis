...
mini_snorkel_raw |> 
  filter(count > 0) |> 
  ggplot() + 
  geom_histogram(aes(count, fill = channel_location), binwidth = 100) + 
...
hurdle_data <- mini_snorkel_model_ready |>
  ...
  select(count, depth, velocity, cobble_substrate:large_woody_cover_more_than_half_meter_overhead, month, fl_mm) |>
...
hurdle_model <- pscl::hurdle(count ~ small_woody_cover_inchannel + depth + velocity + month + large_woody_cover_inchannel + submerged_aquatic_veg_inchannel + cover_more_than_half_meter_overhead + half_meter_overhead + cobble_substrate + boulder_substrate + undercut_bank, data = hurdle_data, dist = "negbin") 
...
hurdle_data <- mini_snorkel_model_ready |>
  ...
  mutate(cobble_substrate = ifelse(percent_cobble_substrate >= percent_threshold, 1, 0 ),
         ...
         large_woody_inchannel_and_half_meter_overhead = ifelse(large_woody_inchannel_and_more_than_half_meter_overhead >= percent_threshold, 1, 0),
         large_woody_cover_more_than_half_meter_overhead = ifelse(large_woody_inchannel_and_more_than_half_meter_overhead >= percent_threshold, 1, 0)) |>
  filter(channel_location == "HFC") |> 
  select(count, depth, velocity, cobble_substrate:large_woody_cover_more_than_half_meter_overhead, month, channel_location) |>
  # remove no cover variables (no cover overhead, no cover in channel)
  select(-contains("no_cover")) |> 
  distinct() |> 
  na.omit() |> 
...
plot_model(best_model_hurdle, title = "Figure X. HFC Hurdle Model Results Using 20% Threshold for Explanatory Variable Values", wrap.title = 50) + 
    scale_color_manual(values = custom_colors[5:6])
...
plot_model(best_model_hurdle, title = "Figure X. LFC Hurdle Model Results Using 20% Threshold for Explanatory Variable Values", wrap.title = 50) + 
    scale_color_manual(values = custom_colors[5:6])
...
-   Model 1 has much a much larger range in the incidence result ratio,
    implying that each significant predictor has a stronger impact on
    fish count and fish presence than in Model 2 or Model 3
...
-   The use of one model or the other is dependent on the input dataset
    ...
    presence.
-   Adding the 2002 data modified the results
